 ...
 */
function isTaskScheduledForDate(task, targetDate, targetDay, targetDayOfWeek, targetMonth, targetYear, weekOfMonth) {
  ...
  return patterns.some(pattern => {
    const { frequency, interval, days, weekOfMonth: patternWeekOfMonth, dayOfWeek } = pattern;
    ...
    switch (frequency) {
      case 'daily':
        ...
        const taskStartDate = task.recurring.startDate ? new Date(task.recurring.startDate) : new Date(0); // Use epoch if no start date
        const daysSinceStart = Math.floor((targetDate - taskStartDate) / (1000 * 60 * 60 * 24));
      ...
      case 'weekly':
        ...
        const taskWeeklyStartDate = task.recurring.startDate ? new Date(task.recurring.startDate) : new Date(0);
        const weeksSinceStart = Math.floor((targetDate - taskWeeklyStartDate) / (1000 * 60 * 60 * 24 * 7));
        
        // Check if this is the right week interval and the right day of week
        return weeksSinceStart % interval === 0 && days.includes(targetDayOfWeek);
      ...
      case 'monthly':
        ...
        if (days && days.length > 0) {
          ...
          const startDate = task.recurring.startDate ? new Date(task.recurring.startDate) : new Date(0);
          const startMonthsSinceEpoch = startDate.getFullYear() * 12 + startDate.getMonth();
...
function determineAssignee(task, targetDate) {
  ...
  if (task.recurring && task.recurring.patterns && task.recurring.patterns.length > 0) {
    ...
    switch (pattern.frequency) {
      case 'daily':
        ...
        const daysSinceStart = Math.floor((targetDate - taskStartDate) / (1000 * 60 * 60 * 24));
        rotationIndex = Math.floor(daysSinceStart / pattern.interval) % rotationOrder.length;
        break;
        
      case 'weekly':
        // Calculate weeks since start
        const weeksSinceStart = Math.floor((targetDate - taskStartDate) / (1000 * 60 * 60 * 24 * 7));
        rotationIndex = Math.floor(weeksSinceStart / pattern.interval) % rotationOrder.length;
        break;
        
      case 'monthly':
        // Calculate months since start
        const targetMonthsSinceEpoch = targetDate.getFullYear() * 12 + targetDate.getMonth();
        const startMonthsSinceEpoch = taskStartDate.getFullYear() * 12 + taskStartDate.getMonth();
        const monthsSinceStart = targetMonthsSinceEpoch - startMonthsSinceEpoch;
        rotationIndex = Math.floor(monthsSinceStart / pattern.interval) % rotationOrder.length;
        break;
  ...
  const assignee = task.participants.find(participant => 
